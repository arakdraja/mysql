...
var CompressedPacketHeader = require('./CompressedPacketHeader');
...
function Parser(options) {
  ...
  this._compressedBuffers          = null;
  this._compressedPacketHeader     = null;
  ...
  this._nextCompressedPacketNumber = 0;
...
Parser.prototype.write = function write(chunk) {
  if (this._compressedBuffers) {
    this._compressedBuffers.push(chunk);
  } else {
...
Parser.prototype.incrementCompressedPacketNumber = function incrementCompressedPacketNumber() {
  var currentCompressedPacketNumber = this._nextCompressedPacketNumber;
  this._nextCompressedPacketNumber = (this._nextCompressedPacketNumber + 1) % 256;

  return currentCompressedPacketNumber;
};
...
Parser.prototype._advanceToNextPacket = function _advanceToNextPacket() {
...
Parser.prototype._combineCompressedBuffers = function _combineCompressedBuffers(bytes) {
  if (this._compressedBuffers.size < bytes) {
    return null;
  }

  var buffers = new BufferList();

  while (buffers.size < bytes) {
    buffers.push(this._compressedBuffers.shift());
  }

  var buffer = Buffer.allocUnsafe(buffers.size);
  var offset = 0;
  while (buffers.size) {
    offset += buffers.shift().copy(buffer, offset);
  }
  ...
  this._compressedBuffers.unshift(buffer.slice(bytes));
...
Parser.prototype._startInflate = function _startInflate() {
  if (this._compressed) {
    return;
  }

  this._compressed        = true;
  this._compressedBuffers = new BufferList();

  var trailingPacketBytes = this._buffer.length - this._packetEnd;

  if (trailingPacketBytes !== 0) {
    this._compressedBuffers.push(this._buffer.slice(this._packetEnd));
    this._buffer = this._buffer.slice(0, this._packetEnd);
  }

  var buf = null;
  while ((buf = this._nextBuffers.shift())) {
    this._compressedBuffers.push(buf);
  }

  // TODO: trigger write loop
};

Parser.prototype._tryInflateNextPacket = function _tryInflateNextPacket() {
  if (!this._compressed || this._inflating) {
    return false;
  }

  var compressedPacketHeader = this._tryReadCompressedPacketHeader();

  if (!compressedPacketHeader) {
    return false;
  }

  var compressedPacketData = this._combineCompressedBuffers(compressedPacketHeader.length);

  if (!compressedPacketData) {
    return false;
  }

  this._compressedPacketHeader = null;

  if (compressedPacketHeader.size === 0) {
    this._nextBuffers.push(compressedPacketData);
    return true;
  }

  var self = this;
  this._inflating = true;
  Zlib.inflate(compressedPacketData, function(err, data) {
    if (err) {
      err.fatal = true;
      self.onError(err);
      return;
    ...
    self._nextBuffers.push(data);
    self.write();
  });
  return false;
};

Parser.prototype._tryReadCompressedPacketHeader = function _tryReadCompressedPacketHeader() {
  if (this._compressedPacketHeader) {
    return this._compressedPacketHeader;
  }

  var headerBuffer = this._combineCompressedBuffers(COMPRESSED_PACKET_HEADER_LENGTH);

  if (!headerBuffer) {
    return null;
  }

  this._compressedPacketHeader = CompressedPacketHeader.fromBuffer(headerBuffer);

  var expectedSequence = this.incrementCompressedPacketNumber();
  if (this._compressedPacketHeader.sequence !== expectedSequence) {
    var err = new Error(
      'Compressed packets out of order. Got: ' + this._compressedPacketHeader.sequence + ' ' +
...
Parser.prototype._tryReadPacketHeader = function _tryReadPacketHeader() {
  ...
  var expectedNumber = this.incrementPacketNumber();
  if (this._packetHeader.number !== expectedNumber) {
    if (this._compressed && this._packetHeader.number === (this._nextCompressedPacketNumber - 1)) {
      this._nextPacketNumber = this._nextCompressedPacketNumber;
    } else {
      var err = new Error(
